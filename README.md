...
Production использует один контейнер приложения: multi-stage сборка компилирует
React/Vite, а FastAPI раздаёт `frontend/dist`, API и health endpoints с одного
origin. Единственное внешнее состояние — PostgreSQL (в целевой конфигурации
Neon), подключаемый через `DATABASE_URL`.

Перед появлением телефонной авторизации импорт и изменение каталога по умолчанию
отключены (`PATTERN_CREATION_ENABLED=false`). Каталог, редактор и клиентский
экспорт остаются публичными. Миграции и seed запускаются автоматически перед
Uvicorn. MVP рассчитан на один Railway instance и один Uvicorn worker;
горизонтальное масштабирование в этот этап не входит.

Полная последовательность локальной production-проверки, будущего deploy,
rollback и восстановления backup приведена в
